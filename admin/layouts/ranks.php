...
$allowperm = $rankresult["RANK_LIST"];
...
$page   = @ceil($_GET['page']);
if ($page < 1) { $page = 1;}
$Say   = $conn->query("select * from ranks order by id DESC");
...
$visiblepage   = 100;
if ($AllResult < 1) {
    $check_data	= $conn->query("select * from ranks order by id DESC");
}
else {
    $check_data	= $conn->query("select * from ranks order by id DESC limit $Goster,$Limit");
}$ranks = $check_data->fetchAll(PDO::FETCH_OBJ);
$q = $conn->prepare("DESCRIBE ranks");
$q->execute();
...
<div class="container-fluid">
    <br>
    <div class="btn-group">
        <a href="./create/ranks.php" class="btn btn-outline-primary btn-sm disabled">Yetki Oluştur</a>
    </div>
    <div class="btn-group">
        <a href="./users.php" class="btn btn-outline-primary btn-sm">Kullanıcıları Görüntüle</a>
    </div>
    <br> <br>
        ...
        <div class="col">
            <table class="table table-bordered table-striped table-sm table-responsive text-center">
                ...
                foreach($ranks as $rank) {
                    ...
                    <td><?= $rank->name ?></td>
                        <td hidden="hidden">
                            <a href="./update/ranks.php?pid=<?= $rank-> id ?>" class="btn btn-outline-warning disabled" >Güncelle</a>
                            <a href="./delete/ranks.php?pid=<?= $rank-> id ?>" class="btn btn-outline-danger disabled">Sil</a>
