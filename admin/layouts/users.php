...
$allowperm = $rankresult["USER_LIST"];
...
$page   = @ceil($_GET['page']);
if ($page < 1) { $page = 1;}
$Say   = $conn->query("select * from users order by id DESC");
...
$visiblepage   = 100;
if ($AllResult < 1) {
	$check_data	= $conn->query("select * from users order by id DESC");
}
else {
	$check_data	= $conn->query("select * from users order by id DESC limit $Goster,$Limit");
}$users = $check_data->fetchAll(PDO::FETCH_OBJ);
?>
<body>
<div class="container-fluid">
    <br>
    <div class="btn-group">
        <a href="./create/users.php" class="btn btn-outline-primary btn-sm">Kullanıcı Oluştur</a>
    </div>
    <div class="btn-group">
        <a href="./ranks.php" class="btn btn-outline-primary btn-sm">Yetkileri Görüntüle</a>
    </div>
    <br> <br>
        ...
        <div class="col">
            <table class="table table-bordered table-striped table-sm table-responsive text-center">
                ...
                foreach($users as $user) {
                    $rank_prepare = "SELECT id, name FROM ranks WHERE id = :id";
                    $ranks = $conn->prepare($rank_prepare);
                    $ranks->bindValue(':id', $user->rank);
                    $ranks->execute();
                    $rankname = $ranks->fetch(PDO::FETCH_ASSOC);?>
                    <tr>
                        <td><?= $user->id ?></td>
                        <td><?= $user->username ?></td>
                    <td><?= $user->registerdate ?></td>
                        ...
                        <td>
                            <a href="./update/users.php?pid=<?= $user-> id ?>" class="btn btn-outline-warning">Güncelle</a>
                            <a href="./delete/users.php?pid=<?= $user-> id ?>" class="btn btn-outline-danger">Sil</a>
